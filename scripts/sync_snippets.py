...
Path resolution:
- Paths starting with ./ or ../ are relative to the containing file
...
def resolve_source_path(source_path: str, containing_file: Path, repo_root: Path) -> Path:
    ...
    if source_path.startswith(("./", "../")):
        return (containing_file.parent / source_path).resolve()
...
class SnippetSyncer:
    ...
    def get_source_content(self, source_ref: str, containing_file: Path) -> str:
        ...
        resolved = resolve_source_path(file_path_str, containing_file, self.repo_root)
    ...
    def process_file(self, file_path: Path, *, check: bool = False) -> ProcessingResult:
        ...
        def replace_snippet(match: re.Match[str]) -> str:
            ...
            try:
                code = self.get_source_content(source_ref, file_path)
